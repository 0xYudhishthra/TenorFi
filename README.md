...
- **Speculator** (a.k.a. *Volátil*) receives floating / pays fixed → bets funding stays high (buys upside).
- **Keel matches + custodies** both sides — no house position. It never touches your Hyperliquid position; it settles against a *public number* (the funding rate, read by Chainlink CRE) — like rain insurance pays on rainfall without controlling the weather. Funding is capped per period and collateral is pre-locked to cover the max, so **the most anyone can owe is already paid → no default.**
