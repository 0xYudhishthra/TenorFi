...
- **The insurer = us = the rate-offerer (the *maker*), backed by the protocol's pre-funded reserve.** We post **consistent fixed-rate offers** and stand as the counterparty, so a hedger can lock **instantly** without waiting for anyone to show up. We take the opposite leg — **receive fixed, pay floating** — so we earn the premium when funding stays below the locked rate and pay out (bounded per period by the cap) when it spikes. The *certainty-seller*. *(Mechanically this is an LP/market-maker position; we frame it to users as the insurer.)*
- **Speculator (*Volátil*) — deferred to phase 2, NOT built.** In the mature market, people who want to bet on funding take the reserve's side and the protocol only fills the gaps. **For now, we ARE the counterparty.**
...
**Cold-start is the real problem this solves.** A swap needs a counterparty, and at any moment there may be none. By making the **insurer's reserve the always-on rate-offerer**, a hedger gets certainty on day one — the missing piece that turns the idea into a working product.
...
flowchart TB
    ...
    FLOW --> OPEN["Open Keel position (KeelSwap.open) → capital locked"]
...
- **1inch Aqua / SwapVM** — ✅ **BUILT + TESTED + AUDITED** (`packages/contracts/src/swapvm`). A custom `_fundingSettle` SwapVM instruction (`amountOut = clamp(R−F,±cap)×N`) registered in our own router (`KeelSwapVMRouter`); virtual balances keep collateral in-wallet. Settlement is **directional and taker-bound**: SwapVM is one-directional, so a position is **two mirror orders** (`makerPaysAbove` true/false), each paying 0 outside its own direction and reverting `UnauthorizedTaker` if anyone but the agreed counterparty takes it. Unit-tested + an **e2e where a settlement executes through the opcode and moves real USDC via Aqua**, a **double-settle guard** (per order+period), a deploy-wiring test, and a **Base mainnet fork test against the real deployed Aqua + real USDC** — **50 tests** (48 offline + 2 Base-mainnet fork), plus a Slither pass and an adversarial audit (`docs/security-review.md`). Live Base mainnet deploy pending. Each period is one atomic, keeper/CRE-triggered swap; recurrence is external.
...
- **`_fundingSettle(Context, bytes)`** — the one custom SwapVM instruction: reads the latched funding index + swap terms (fixed rate, notional, counterparty, direction), computes the net cashflow `(realized − fixed) × notional` (capped at the hourly clamp), and delivers it from payer → receiver. **Directional** (a position is two mirror orders, each paying only in its own direction) and **taker-bound** (`UnauthorizedTaker`). Marks the period settled (no double-settle).
...
- **Open / close** = ordinary swaps (pull collateral → mint position tokens / return collateral). `close()` is **pull-over-push**: each party `withdraw()`s independently (resilient to a USDC blacklist freezing one side).
- **MVP scope:** one swap between a **hedger (customer/taker)** and the **protocol's insurance reserve (maker/counterparty — us, in the MVP)** + the CRE index + a keeper firing `settle()` each period. The hedger takes our offered fixed rate via the MCP and confirms in-app. **That alone demos the thesis.** The **`_fundingSettle` SwapVM opcode is built + tested** (`packages/contracts/src/swapvm`); `KeelSwap` (plain Solidity) is the equivalent fallback. **Independent speculators replacing the team-run reserve are phase 2.** *(SwapVM has no native scheduling → the settle is keeper/CRE-triggered each period.)*

> **End-to-end flow (user + system + the per-period loop): see [`flows.md`](flows.md).**

### Settlement math (formal spec — reconciled with the shipped `KeelSwap`, source of truth: 50 tests green)
...
- **Per-period net cashflow:** `payment = N × clamp(R − F, ±cap)` (the **credit to the hedger**). Equivalently, the hedger's *outflow* is `N × (F − R)`.
  - **Direction (canonical, from the tested contract):** `net = realized − fixed`. `R > F` → **hedger is credited, the insurance reserve (counterparty) pays**; `R < F` → **hedger pays the premium, the reserve is credited**. (In `KeelSwap` the counterparty fills the `speculator` parameter slot — in the MVP that address is the protocol's pre-funded reserve, run by us.) ⚠️ An earlier draft wrote *"F > R → Hedger receives,"* which is **wrong** — it inverts the leg. The shipped contract uses `net = realized − fixed` (hedger = floating receiver), and that is the source of truth.
...
## 7. User flows
> **Canonical, current flow (user + system + per-period loop, with diagrams): [`flows.md`](flows.md).** The summary below is kept for context.
...
- **First-3-hours validation gate (fail fast):** (1) can CRE fetch Hyperliquid BTC funding and write the index on-chain to the consumer on Base mainnet? → else EOA relayer fallback. (2) the custom SwapVM opcode → ✅ already built + tested (`packages/contracts/src/swapvm`). (3) can the contract read the index and transfer between two parties in one period? → ✅ proven (`KeelSwap` + opcode tests).
- **Build long poles (start now):** the `_fundingSettle` opcode + the fixed-rate AMM curve; the CRE funding-oracle workflow; the hourly settlement + collateral/cap loop (✅ settlement core + opcode done — 50 tests green, Slither + adversarial audit passed).
