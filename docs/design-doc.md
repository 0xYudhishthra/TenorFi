...
- **Insurance reserve = us = the rate-offerer (the *maker*).** We post **consistent fixed-rate offers** and stand as the counterparty, so a hedger can lock **instantly** without waiting for anyone to show up. We take the opposite leg — **receive fixed, pay floating** — so we earn the premium when funding stays below the locked rate and pay out (bounded per period by the cap) when it spikes. The *certainty-seller*.
...
- **Why it survives a black swan — three locks:**
  1. **The protocol is neutral; the reserve's risk is bounded.** The Keel *contract* holds no position — it only custodies and settles. Our **insurance reserve** is the counterparty and *does* take directional risk, but it is **bounded**: per period it can never owe more than `cap × notional`, and that max is pre-locked up front.
...
- **Honest residual risks (say them):** as the **insurance reserve we take bounded directional risk** (capped per period, not zero — we earn when funding stays calm, pay when it spikes) — Voltz-style; **oracle staleness under congestion**; **correlated collateral.** Owning these makes you more credible.
...
flowchart TB
    ...
    RESERVE["Insurance reserve (us / maker) — offers the fixed rate<br/>(speculators take this side in phase 2)"] --> KEEL
    ...
    KEEL["KEEL — funding-rate swap on 1inch Aqua / SwapVM (Base mainnet)<br/>custom _fundingSettle opcode (a derivative, not a trade) — BUILT + TESTED<br/>collateral stays alive via Aqua virtual balances; the reserve is the bounded counterparty"]
    KEEL -->|settle / payout in USDC| OUT["Hedger ↔ reserve via Aqua virtual balances"]
...
The "3 fixed-rate positions" are **the reserve's standing offers** — each is a `fixed rate` + a `max coverage` (where coverage = `cap × notional`, the most the insurance pays out). The user picks one and **confirms to open** (a single in-app signature).
...
- **`AFR < FFR`** → funding is below the locked rate → **the premium flows from the user's Keel collateral into the insurance reserve** (the cost of certainty).
...
flowchart TB
    ...
    LOOP -->|"AFR < FFR"| PREM["premium from user → insurance reserve"]
    UP --> LOOP
    PREM --> LOOP
...
### Settlement math (formal spec — pinned by the tested `_fundingSettle` opcode, source of truth: 35 tests green)
...
- **Primary entry path (Chat on MCP → LI.FI Composer → live hedge):**
  ...
  2. **Pick an offer** — the MCP lists the reserve's standing offers (`fixed rate` + `max coverage = cap × notional`); the user picks one and confirms with **one signature**.
