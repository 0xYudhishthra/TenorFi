...
| Decision | Choice | Why |
|---|---|---|
| Chain | **Base Sepolia**; local **anvil** as demo fallback | oracle source = settlement chain; anvil if testnet is flaky |
...
1. **(T+0:00–0:30) Deploy.** Deploy `MockUSDC`, `FundingIndex`, `KeelSwap` to Base Sepolia. Write `deployments.json` + ABIs. Faucet USDC to demo wallets. **Publish to the team.**
...
### Axel — Chainlink CRE
1. **(0:00–0:30) Recon.** Base Sepolia RPC + check CRE KeystoneForwarder availability. Get the `FundingIndex` address from You.
...
| Time | Milestone | Gate |
|---|---|---|
| 17:00 | **Standup** — lock §0 decisions, assign, confirm Ledger hw + Base Sepolia RPC | go/no-go on chain |
...
|---|---|---|
| CRE slow / forwarder missing on Base Sepolia | not writing by 19:30 | EOA relayer posts the real API-derived index; keep 1 CRE write for the bounty |
| Base Sepolia down / RPC flaky | deploy fails in first 30 min | Base Sepolia, else local **anvil** for the demo (we need no EIP-1153 — plain Solidity) |
...
1. **Open via conversation:** User tells the MCP: *"Create a long on BTC on Hyperliquid and fix the funding rate."* MCP reads available offers, presents options, user picks one. **Agent proposes → user signs on Ledger → both legs open** (Hyperliquid perp + Keel swap, real txs on Base Sepolia).
...
**Be honest on screen:** the swap, settlements, and Ledger signatures are real (Base Sepolia); the Ethena comparison uses real historical funding data in the narration.
...
- [ ] Architecture diagram (HL API → MCP → CRE → KeelSwap → Ledger, on Base Sepolia)
