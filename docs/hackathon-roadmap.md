...
> deployed + Basescan-verified); the custom SwapVM opcode is built (39 passing tests, Slither + adversarial audit).
> Kept for the record.

**Window:** 17:00 → 01:00 (submit early — treat 00:15 as the real deadline).
**Team (4):** Axel (Chainlink CRE) · Tomas (Ledger + math) · Shaun (agent + submission) · You (contracts, backend, integration).
...
**Pivot:** agent-first demo (agent-driven interaction) instead of UI. The agent is the primary interface showing "agent proposes, human disposes."
...
| Period length | **`PERIOD_SECONDS = 3600`** (hourly — matches Hyperliquid funding + CRE) | a multi-week hedge replays in the demo slot |
...
| Agent | **PRIMARY DEMO INTERFACE** — conversational agent drives Hyperliquid + TenorFi swap, gates brink decision to Ledger | the hero; simple web chat interface for fallback if the agent flaky |
...
DEPLOY contracts (You, first 30 min)  ──unblocks──►  everyone
        ...
        ├─► Agent backend + tools (Shaun: read Hyperliquid, open/settle swap, propose_decision)
        ├─► Ledger signs txs against deployed KeelSwap (Tomas)
        └─► Backend/keeper fires settle each period (You)
                                  │
                          WIRE END-TO-END ──► agent drives swap via conversation ──► Ledger moment ──► demo video ──► SUBMIT
                                  │
                     (fallback: simple web chat if the agent flaky)
```
...
- **Period numbering:** `period = floor(unixSeconds / PERIOD_SECONDS)`. Axel writes the index keyed by `period`; the contract reads it; the agent displays it. **Everyone uses this exact formula.**
...
- **Swap state for the agent:** `swaps(id)` getter + events `SwapOpened`, `Settled(swapId, period, realized, diff, amount, payer, receiver)`, `SwapClosed`. `previewSettle(id, realized)` for projections.
- **Brink flag (off-chain):** the agent computes `remaining < cap × notional / 1e18` from the getter → surface the Ledger decision.
- **Ethena replay data:** historical Hyperliquid BTC funding (CSV/JSON) — Axel/You provide; used in demo narrative/comparison.
- **Agent tool signatures:** `get_funding(market)`, `list_offers()`, `get_position(address)`, `open_hyperliquid_position(market, side, size)`, `open_keel_position(offerId)`, `settle(swapId, period)`, `topup_hyperliquid_margin(positionId, amount)`, `propose_decision(swapId)` → returns unsigned tx for Ledger.
...
3. **(1:15–3:30) Backend/keeper.** Tiny indexer/keeper (viem): fires `settle(id, period)` each compressed period (60s), exposes swap state via simple REST API or direct chain reads for the agent. Stand up the **EOA relayer** path so it's ready if CRE slips.
4. **(3:30–6:00) Integration support.** Help Shaun wire agent tools to contracts, provide Hyperliquid testnet API integration guidance, ensure settlement loop + Ledger signing paths work end-to-end. Build simple web chat fallback if the agent integration hits blockers.
...
3. **(1:00–4:00) Ledger signing integration.** Build signing flow for `open` / `close` / `topUp` transactions. Create helper functions that the agent can call to get user signatures. Build the **brink scenario**: drive realized funding extreme so one side nears the floor.
4. **(4:00–6:00) Wire the Ledger moment with Shaun's agent:** brink detected → the agent calls `propose_decision` → presents unsigned tx options (close/continue) → human signs on Ledger → the agent submits signed tx. This is WOW #4.

### Shaun — agent + submission
1. **(0:00–1:00) Agent scaffold.** Set up the agent backend / tool server: basic server, tool registration structure, prepare tool schemas for the 8 core tools (read: `get_funding`, `list_offers`, `get_position`; write: `open_hyperliquid_position`, `open_keel_position`, `settle`, `topup_hyperliquid_margin`; gated: `propose_decision`).
2. **(1:00–3:00) Implement agent tools** (wait for `deployments.json` at 0:30):
...
3. **(3:00–5:00) Wire settlement loop + Ledger integration** (with Tomas):
   ...
   - Test full flow: open position via agent conversation → settlement fires → brink → Ledger signs
4. **(5:00–6:00) Demo polish + submission writeup.** Prepare demo conversation script, record agent-driven flow, write submission highlighting "agent proposes, human disposes" thesis. Start writeup at T+5:00 regardless of agent state; fallback to simple web chat if the agent doesn't stabilize.
...
| 19:00 | Agent backend scaffold live; CRE first write attempt; Ledger signs a test tx | — |
| 19:30 | **CRE checkpoint** — real write or fall back to relayer | live funding source decided |
| 20:00 | **End-to-end thread working**: the agent opens swap → settle a period → the agent reads state | the core demo exists |
| 21:30 | Agent settlement loop working (AFR/FFR comparison → topup) | hero flow locked |
| 22:30 | **Ledger moment** wired via the agent; full conversation flow tested | feature freeze approaches |
| 23:00 | **FEATURE FREEZE** — only bug-fixes on the demo path after this | — |
| 23:00–00:15 | Demo video (agent conversation) + submission writeup + bounty applications | — |
...
| Agent integration blockers (SDK issues, tool wiring) | behind at 21:30 | simple web chat interface calling contract functions directly; demonstrate agent-gated flow via script |
| Hyperliquid testnet API rate limits / flaky | during agent testing | mock Hyperliquid responses in the agent tools; show API integration in code, demo with mocked data |
...
**THE CONVERSATIONAL FLOW (agent-driven, agent proposes / human disposes):**

1. **Open via conversation:** User tells the agent: *"Create a long on BTC on Hyperliquid and fix the funding rate."* The agent reads available offers, presents options, user picks one. **Agent proposes → user signs on Ledger → both legs open** (Hyperliquid perp + TenorFi swap, real txs on Base mainnet).

2. **Live settlement loop:** Compressed periods (60s each) settle automatically. Show the agent monitoring: *"AFR is ~13% APR, FFR is 7.3% APR — funding is high, protocol pays you $X, topping up your Hyperliquid margin."* Real USDC movements on-chain.
...
4. **The Ledger moment (hero):** Drive funding extreme → collateral hits brink. The agent detects it and says: *"Your collateral is low. Options: (1) Close now, (2) Add more collateral and continue. Sign on your Ledger to decide."* **Human picks up Ledger, signs the decision.** *Agent proposes, human disposes* — demonstrated live, not claimed.

5. **Closing beat:** The agent confirms: *"Position closed. You locked certainty when the market swung wild."*
...
- [ ] Demo video (agent conversation flow from script above — show terminal/chat with the agent, Ledger signing moment)
- [ ] Project description + the Ethena hook + **"agent proposes, human disposes" thesis**
- [ ] Architecture diagram (HL API → agent → CRE → KeelSwap → Ledger, on Base mainnet)
- [ ] Repo link + deployed addresses (`deployments.json`) + **agent backend code**
- [ ] Bounty applications:
...
- [ ] Team + roles (Shaun: agent, You: contracts/backend, Axel: CRE, Tomas: Ledger/math)

---

## 8. Explicitly NOT building tonight

**UI (replaced by the agent)** · SwapVM opcode · AMM/LP imbalance backstop · cumulative accumulator · cross-chain collateral · re-match logic (narrated). All are roadmap — say so if asked.

**The pivot:** the agent is the demo interface. A polished rates-trading UI (Ethena replay chart, lock card, settlement feed) is deferred to post-hackathon. Tonight we prove "agent proposes, human disposes" via a conversational agent flow.
