...
## Test posture
- **39 passing tests** (43 total, 4 fork tests skipped without an RPC) — `forge test` (fork tests skip without `BASE_RPC_URL`).
...
| `timestamp` — `FundingSettle` derives `period = block.timestamp / periodSeconds` | Info | **Intentional.** A miner can nudge the timestamp by seconds, not across a 60s period boundary meaningfully; and double-settle is guarded per `(orderHash, period)`. (`_clamp`/`_abs` flags are false positives — no timestamp use.) |
...
| B | High | **Opcode dropped the sign of `R − F`** → maker paid in both directions / drainable | **Fixed** — settlement is now directional via the signed net: a **single order** routes coverage (`amountOut`, `R > F`) or premium pull (`amountIn`, `R < F`) by the sign of `clamp(R − F, ±cap)` (reconciled to deployed opcode — one order serves both directions, not two mirror orders). Tests: `test_wrongDirection_paysZero`, `test_makerPaysBelow_RBelowF_pays`. |
...
**Test count after fixes + KeelSwap removal: 39 passing** (43 total, 4 fork tests skipped without an RPC). Covers: directional settlement, taker-bind, stranger-cannot-take, clamp-to-cap, double-settle, no-default on the Aqua path (ship-floor covers the worst case; underfunded period reverts), and the single-order both-directions settlement (the R<F subscriber-pays-reserve premium pull) end-to-end on real Aqua.
