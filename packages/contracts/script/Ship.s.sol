...
contract Ship is Script {
    ...
    int256 internal constant FIXED_RATE = 1e16; // 1% per period (FFR)
    uint256 internal constant CAP = 4e16; // 4% per period clamp
    ...
    function run() external {
        ...
        ISwapVM.Order memory order = KeelFundingProgram(PROGRAM).buildProgram(
            reserve, FUNDING_INDEX, FIXED_RATE, CAP, notional, PERIOD_SECONDS, hedger, true
        );
        ...
        _writeOrderParams(reserve, hedger, notional, collateral);
    }
    ...
    function _writeOrderParams(
        ...
        vm.serializeInt(k, "fixedRate", FIXED_RATE);
